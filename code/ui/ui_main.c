...
Q_EXPORT intptr_t vmMain( long command, long arg0, long arg1, long arg2, long arg3, long arg4, long arg5, long arg6, long arg7, long arg8, long arg9, long arg10, long arg11  )
{
    switch ( command )
    {
...
// Changed RD
    ...
    uiInfo.uiDC.Assets.capsulename = String_Alloc("xb2capsule");
    uiInfo.uiDC.Assets.bankname = String_Alloc("bank_menu");
    uiInfo.uiDC.Assets.vehiclename = String_Alloc("vehicle_menu");
    uiInfo.uiDC.Assets.adminname = String_Alloc("admin");
...
void Text_Paint_AutoWrapped(float x, float y, float xmax, float ystep, float scale, vec4_t color, const char *str, float adjust, int limit, int style)
{
    int width;
    char *s1,*s2,*s3;
    char c_bcp;
    ...
    while (1)
    {
        ...
        *s3 = c_bcp;
        if (width > xmax)
        {
            ...
            *s2 = '\0';
            Text_Paint(x, y, scale, color, s1, adjust, limit, style);
            y += ystep;
            if (c_bcp == '\0')
            {
                ...
                if (*s2 != '\0') // if we are printing an overflowing line we have s2 == s3
                    Text_Paint(x, y, scale, color, s2, adjust, limit, style);
                break;
            }
            s2++;
            s1 = s2;
...
// Changed RD
        ...
        if (Q_stricmp(token.string, "capsuleMenu") == 0)
        {
            if (!PC_String_Parse(handle, &uiInfo.uiDC.Assets.capsulename))
            {
                return qfalse;
            }
            continue;
        ...
        if (Q_stricmp(token.string, "bankMenu") == 0)
        {
            if (!PC_String_Parse(handle, &uiInfo.uiDC.Assets.bankname))
            {
                return qfalse;
            }
            continue;
        ...
        if (Q_stricmp(token.string, "vehicleMenu") == 0)
        {
            if (!PC_String_Parse(handle, &uiInfo.uiDC.Assets.vehiclename))
            {
                return qfalse;
            }
            continue;
        ...
        if (Q_stricmp(token.string, "adminMenu") == 0)
        {
            if (!PC_String_Parse(handle, &uiInfo.uiDC.Assets.adminname))
            {
                return qfalse;
            }
            continue;
...
static void UI_DrawOpponent(rectDef_t *rect)
{
    ...
    if (updateOpponentModel)
    {
        ...
        UI_PlayerInfo_SetInfo( &info2, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, RWP_FIRE, qfalse );
...
===============
*/
int	UI_ClassforPlayer( char *modelName )
{
    if( !Q_stricmp (modelName, "mmx"))
        ...
        return CLASS_DRN001;
    //CG_Printf("Classnum is equal to %i\n", classnum);
}

static void UI_UpgradeName(rectDef_t *rect, float scale, vec4_t color, int textStyle, int upgrade)
{
    Text_Paint(rect->x, rect->y, scale, color, upgrade_table[upgrade].name, 0, 0, textStyle);
}

static void UI_UpgradeDesc(rectDef_t *rect, float scale, vec4_t color, int textStyle, int upgrade)
{
    Text_Paint_AutoWrapped(rect->x, rect->y, rect->w-10, 20, scale, color, upgrade_table[upgrade].desc, 0, 0, textStyle);
    trap_Cvar_Set("ui_UpgChips", va("%d", upgrade_table[upgrade].price));
}

static void UI_UpgradePrice(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
    Text_Paint_AutoWrapped(rect->x, rect->y, rect->w-10, 20, scale, color, UI_Cvar_VariableString("ui_UpgChips"), 0, 0, textStyle);
}
...
// end changed RD
    ...
    case UI_KEYBINDSTATUS:
        ...
        break;
    case UI_HEAD1:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[0]);
        break;
    case UI_HEAD1_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[0]);
        break;
    case UI_HEAD2:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[1]);
        break;
    case UI_HEAD2_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[1]);
        break;
    case UI_HEAD3:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[2]);
        break;
    case UI_HEAD3_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_head[2]);
        break;
    case UI_BACK1:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[0]);
        break;
    case UI_BACK1_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[0]);
        break;
    case UI_BACK2:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[1]);
        break;
    case UI_BACK2_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[1]);
        break;
    case UI_BACK3:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[2]);
        break;
    case UI_BACK3_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_back[2]);
        break;
    case UI_BODY1:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[0]);
        break;
    case UI_BODY1_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[0]);
        break;
    case UI_BODY2:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[1]);
        break;
    case UI_BODY2_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[1]);
        break;
    case UI_BODY3:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[2]);
        break;
    case UI_BODY3_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_body[2]);
        break;
    case UI_BUSTER1:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[0]);
        break;
    case UI_BUSTER1_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[0]);
        break;
    case UI_BUSTER2:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[1]);
        break;
    case UI_BUSTER2_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[1]);
        break;
    case UI_BUSTER3:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[2]);
        break;
    case UI_BUSTER3_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_buster[2]);
        break;
    case UI_LEGS1:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[0]);
        break;
    case UI_LEGS1_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[0]);
        break;
    case UI_LEGS2:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[1]);
        break;
    case UI_LEGS2_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[1]);
        break;
    case UI_LEGS3:
        UI_UpgradeName(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[2]);
        break;
    case UI_LEGS3_DESC:
        UI_UpgradeDesc(&rect,scale, color, textStyle, bg_playerlist[UI_ClassforPlayer(UI_Cvar_VariableString("model"))].ug_legs[2]);
        break;
    case UI_UGPRICE:
        UI_UpgradePrice(&rect,scale, color, textStyle);
        break;
    default:
        break;
    }
...
static void UI_LoadSave( void )
{
    ...
    Com_sprintf(saveExt, sizeof(saveExt), "svg");
...
static void UI_Update(const char *name)
{
    ...
    {
        // Peter: Temp fix
        if (*UI_Cvar_VariableString("ui_Name"))
            trap_Cvar_Set( "name", UI_Cvar_VariableString("ui_Name"));
    }
    else if (Q_stricmp(name, "ui_SetPassword") == 0)
    {
        trap_Cvar_Set( "password", UI_Cvar_VariableString("ui_Password"));
    }
    ...
    else if (Q_stricmp(name, "ui_GetPassword") == 0)
    {
        trap_Cvar_Set( "ui_Password", UI_Cvar_VariableString("password"));
    }
...
qboolean SP_LoadGame(char *load_game, char *loadmap)
{
    ...
    //Com_sprintf( filename, MAX_QPATH, "save\\current.svg");
...
// Changed RD
        ...
        {
            trap_Cvar_Set( "ui_LoadGame", uiInfo.saveList[uiInfo.saveIndex]);
        }
        else if (Q_stricmp(name, "SaveGame") == 0)
        {
            trap_Cvar_Set( "ui_SaveGame", uiInfo.saveList[uiInfo.saveIndex]);
        }
        else if (Q_stricmp(name, "LoadSPGame") == 0)
        {
            char load_game[100];
            char loadmap[100];
            ...
            if (load_game[0])
            {
                if(SP_LoadGame(load_game, loadmap))
                {
                    trap_Cvar_Set("g_gametype", va("%d", GT_SINGLE_PLAYER));
                    trap_Cvar_Set("Save_Loading", "1");
                    trap_Cmd_ExecuteText( EXEC_APPEND, va("spmap %s\n", loadmap));
                }
            }
        }
        else if (Q_stricmp(name, "SaveSPGame") == 0)
        {
            char save_game[100];
            trap_Cvar_VariableStringBuffer( "ui_SaveGame", save_game, sizeof(save_game) );
            if (save_game[0])
            {
                trap_Cmd_ExecuteText( EXEC_APPEND, va("savegame %s\n", save_game));
            }
        }
        else if (Q_stricmp(name, "StartSPGame") == 0)
        {
            if (String_Parse(args, &name2))
            {
...
// end changed RD
        ...
        {
            //
            trap_Cmd_ExecuteText( EXEC_NOW, va( "upgrade %s\n", UI_Cvar_VariableString( "ui_Upgrade" ) ) );
        }
    ...
    for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
    {
        ...
        for (j=0; j<numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS; j++,fileptr+=filelen+1)
        {
            ...
            if (Q_stricmpn(skinname, "icon_", 5) == 0 && !(Q_stricmp(skinname,"icon_blue") == 0 || Q_stricmp(skinname,"icon_red") == 0))
            {
                ...
                if (strcmp(dirptr, "mmx") && strcmp(dirptr, "megaman") &&
                        strcmp(dirptr, "forte") && strcmp(dirptr, "proto") &&
                        strcmp(dirptr, "zero") && strcmp(dirptr, "zero_s") &&
                        strcmp(dirptr, "vile") && strcmp(dirptr, "burnerman"))
                    continue;
...
void _UI_Init( qboolean inGameLoad )
{
    ...
    UI_LoadMenus("ui/capsule.txt", qfalse);
...
// Changed RD
    ...
    trap_Cvar_Set("ui_Weapon", va("%d", RWP_FIRE));
...
void _UI_SetActiveMenu( uiMenuCommand_t menu )
{
    ...
    if (Menu_Count() > 0)
    {
        ...
        case UIMENU_CAPSULE:
            trap_Key_SetCatcher( KEYCATCH_UI );
            Menus_CloseAll();
            Menus_ActivateByName(uiInfo.uiDC.Assets.capsulename);
            return;
        case UIMENU_BANK:
            trap_Key_SetCatcher( KEYCATCH_UI );
            Menus_CloseAll();
            Menus_ActivateByName(uiInfo.uiDC.Assets.bankname);
            return;
        case UIMENU_VEHICLES:
            trap_Key_SetCatcher( KEYCATCH_UI );
            Menus_CloseAll();
            Menus_ActivateByName(uiInfo.uiDC.Assets.vehiclename);
            return;
        case UIMENU_ADMIN:
            trap_Cvar_Set( "cl_paused", "1" );
            trap_Key_SetCatcher( KEYCATCH_UI );
            ...
            Menus_CloseAll();
            // Changed RD
            Menus_ActivateByName(uiInfo.uiDC.Assets.adminname);
            // end changed RD
            return;
        }
    }
...
static cvarTable_t		cvarTable[] =
{
    ...
    { &ui_findPlayer, "ui_findPlayer", "X", CVAR_ARCHIVE},
