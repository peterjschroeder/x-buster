...
// end changed RD
    ...
    if (key == K_MOUSE1 && down && Rect_ContainsPoint(&menu->window.rect, DC->cursorx, DC->cursory) &&
            IS_SET(menu->window.flags, WINDOW_DRAGGABLE))
        SET_BIT(menu->window.flags, WINDOW_DRAG);
    else
        REMOVE_BIT(menu->window.flags, WINDOW_DRAG);
...
// end changed RD
    ...
    // Peter: Draggable windows
    if (IS_SET(menu->window.flags, WINDOW_HASFOCUS) && IS_SET(menu->window.flags, WINDOW_DRAG))
    {
        menu->window.rect.x +=  DC->mx;
        menu->window.rect.y +=  DC->my;
...
qboolean MenuParse_draggable( itemDef_t *item, int handle )
{
    menuDef_t *menu = (menuDef_t*)item;
    menu->window.flags |= WINDOW_DRAGGABLE;
    return qtrue;
}
...
// changed RD
    ...
    {"draggable", MenuParse_draggable, NULL},
