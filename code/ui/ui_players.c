...
//Shinster - no skipping
        /*		if ( i == LEGS_WALKCR ) {
        			skip = animations[LEGS_WALKCR].firstFrame - animations[BOTH_GESTURE].firstFrame;
        		}
        		if ( i >= LEGS_WALKCR ) {
...
void UI_PlayerInfo_SetInfo( playerInfo_t *pi, int legsAnim, int torsoAnim, vec3_t viewAngles, vec3_t moveAngles, weapon_t weaponNumber, qboolean chat )
{
    ...
    if ( torsoAnim == BOTH_DEATH || legsAnim == BOTH_DEATH )
    {
        torsoAnim = legsAnim = BOTH_DEATH;
    ...
    else if ( ( currentAnim == BOTH_GESTURE || currentAnim == TORSO_ATTACK ) && ( torsoAnim != currentAnim ) )
    {
        pi->pendingTorsoAnim = torsoAnim;
    }
