...
Peter: Adjusted so maxChars doesn't cut the line, it wraps it
==================
*/
                       ...
                       qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars )
{
    ...
    if (maxChars <= 0)
        maxChars = strlen(string); // do them all!, Peter: strlen seems
    // cleaner than some number
    ...
    if (shadow)
    {
        ...
        while ( *s )
        {
            ...
            // Peter: If we hit maxChars let's reset x and scale y down
            if ( ( isspace(*s) && cnt >= maxChars ) || (*s == '\n'))
            {
                y += charHeight;
                xx = x;
                cnt = 0;
            ...
            // Don't draw or scale x from escape characters
            if (*s != '\r' && *s != '\n')
            {
