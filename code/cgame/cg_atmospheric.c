...
static void CG_EffectGust(void)
{
    ...
    cg_atmFx.baseEndTime  	  	= cg.time  	  	  	  	  	+ cg_atmFx.baseMinTime  	  	+ (genrand_int32() % (cg_atmFx.baseMaxTime - cg_atmFx.baseMinTime));
    diff  	  	  	  	  	  	= cg_atmFx.changeMaxTime  	- cg_atmFx.changeMinTime;
    cg_atmFx.gustStartTime  	  	= cg_atmFx.baseEndTime  	  	+ cg_atmFx.changeMinTime  	+ (diff ? (genrand_int32() % diff) : 0);
    diff  	  	  	  	  	  	= cg_atmFx.gustMaxTime  	  	- cg_atmFx.gustMinTime;
    cg_atmFx.gustEndTime  	  	= cg_atmFx.gustStartTime  	+ cg_atmFx.gustMinTime  	  	+ (diff ? (genrand_int32() % diff) : 0);
    diff  	  	  	  	  	  	= cg_atmFx.changeMaxTime  	- cg_atmFx.changeMinTime;
    cg_atmFx.baseStartTime  	  	= cg_atmFx.gustEndTime  	  	+ cg_atmFx.changeMinTime  	+ (diff ? (genrand_int32() % diff) : 0);
}
...
void CG_EffectParse( const char *effectstr )
{
    ...
    for( startptr = workbuff; *startptr; )
    {
        ...
        *eqptr++ = 0;
        ...
        if( !type )
        {
            ...
            {
                cg_atmFx.numDrops = 0;
                CG_Printf( NULL, "Atmospheric effect must start with a type.\n" );
                return;
            }
            ...
            else
            {
                if (cg_atmFx.ParticleRender == &CG_RainParticleRender)
                    trap_S_StartSound( cg.snap->ps.origin, cg.clientNum, CHAN_AUTO, trap_S_RegisterSound( "sound/world/rainstop.wav", qfalse ) );
        ...
        else
        {
            ...
            else CG_Printf( NULL, "Unknown effect key '%s'.\n", startptr );
        }
        startptr = endptr;
    }
    ...
    for( count = 0; count < cg_atmFx.numDrops; count++ )
        cg_atmFx.particles[count].nextDropTime = ATMOSPHERIC_DROPDELAY + (genrand_int32() % ATMOSPHERIC_DROPDELAY);
...
void CG_AddAtmosphericEffects()
{
    ...
    for( curr = 0; curr < max; curr++ )
    {
        ...
        if( !cg_atmFx.ParticleCheckVisible( particle ) )
        {
            ...
            if( !particle->nextDropTime )
            {
                ...
                particle->nextDropTime = genrand_int32() % ATMOSPHERIC_DROPDELAY;
            }
    ...
    {
        vec3_t pos;
        VectorSet(pos, cg.refdef.vieworg[0]+crandom()*1024.0f, cg.refdef.vieworg[1]+crandom()*1024.0f, cg.refdef.vieworg[2]);

        if(rand_number(1, 300) < 2 )
            trap_S_StartSound ( pos, 0, CHAN_AUTO, trap_S_RegisterSound(va("sound/world/th%d.wav", rand_number(1, 5)), qfalse) );

        trap_S_AddLoopingSound( cg.clientNum, cg.snap->ps.origin, vec3_origin,
                                cgs.media.rainSound );
    }
}
