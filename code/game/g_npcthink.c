...
retry:
    ...
    else
    {
        ...
        VectorScale(dir,(genrand_int32()%4)+2,dir);
        VectorCopy(ent->r.currentOrigin,from_obj);
...
//		G_Printf( "current altitude: %d     inflight = %d\n",npc->altitude,npc->inFlight);
    ...
    {
        NPC_CheckAround(ent);
        npc->checkTime=level.time+500+genrand_int32()%500;
    }
    ...
    case NPC_COM_ATTACK:
    {
        ...
        {
            NPC_FindTarget(ent);
            if (fabs(npc->ps.viewangles[1]-npc->ideal_angle)<(genrand_int32()%7) || 	ent->npc->npcType==NPC_SEALORD)
            {
                npc->toFire=0;
//					G_Printf( "melee dist: %d   enemy_dist: %d",ent->npc->melee_dist,enemy_dist);
                ...
                else if (ent->npc->npcType==NPC_SEALORD)
                {
                    ...
                    {
                        npc->ps.legsAnim=((npc->ps.legsAnim & ANIM_TOGGLEBIT)^ANIM_TOGGLEBIT) | ANPC_ATTACK1;
                        npc->ps.legsTimer=ent->npc->animTimes[ANPC_ATTACK1];
                        npc->dontMoveTime=level.time+ent->npc->animTimes[ANPC_ATTACK1];
                        npc->fireTime=level.time+ent->npc->animTimes[ANPC_ATTACK1];
                    }
                }
                         ...
                         || (ent->npc->npcType==NPC_BAT && npc->inFlight))	// bat fire from air
                {
                    npc->ps.legsAnim=((npc->ps.legsAnim & ANIM_TOGGLEBIT)^ANIM_TOGGLEBIT) | ANPC_ATTACK_MELEE;
                    npc->ps.legsTimer=ent->npc->animTimes[ANPC_ATTACK_MELEE];
                    npc->dontMoveTime=level.time+ent->npc->animTimes[ANPC_ATTACK_MELEE];
                ...
                else
                {
                    if (ent->npc->npcType==NPC_METLAR)		// metlar jumps instead of firing
                    {
                        npc->ps.legsAnim=((npc->ps.legsAnim & ANIM_TOGGLEBIT)^ANIM_TOGGLEBIT) | ANPC_ATTACK1;
                        npc->ps.legsTimer=ent->npc->animTimes[ANPC_ATTACK1];
                        npc->dontMoveTime=level.time+ent->npc->animTimes[ANPC_ATTACK1]*0.8;
                        npc->toFire=2;
                    ...
                    else
                    {
                        ...
                        npc->ps.legsTimer=ent->npc->animTimes[ANPC_ATTACK1];
                        npc->dontMoveTime=level.time+ent->npc->animTimes[ANPC_ATTACK1];
                        npc->fireTime=level.time+ent->npc->animTimes[ANPC_ATTACK1]*((ent->npc->npcType==NPC_ANK)?0.8:(ent->npc->npcType==NPC_BAT?0.2:0.5));
                        npc->fireCount=0;
                    }
                }
            }
...
//						G_Printf( "my_pos %s       enemy_pos %s\n",vtos(ent->r.currentOrigin),vtos(npc->enemy->r.currentOrigin));
    ...
    case NPC_COM_SEARCH:
    {
        ...
        if (npc->attackTime<=0 || CheckInObj(npc->obj,ent->r.currentOrigin))
        {
            ...
            if (r>0.5 || ent->npc->walkingSpeed==0 || ent->npc->npcType==NPC_BAT)	// look anywhere?
            {
                if (!npc->inFlight)
                {
        ...
        else if (npc->attackTime>0 && ent->npc->walkingSpeed>0 && ent->npc->npcType!=NPC_BAT)
            NPC_MoveLinear(ent,ucmd,NML_WALK);
    }
    break;
    }
