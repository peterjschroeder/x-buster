...
static void NPM_GroundTrace( void )
{
    ...
    if 	( npml.can_do_back_move &&			// only once
            ( ( g_entities[trace.entityNum].s.eType == ET_NPC ) ||
              ( ( g_entities[trace.entityNum].s.eType == ET_PLAYER ) &&
                (npm->ns->toFire != 2 ) ) ) )
    {
...
//		G_Prinuf("dead");
    ...
    if ( npm->npc->npcType==NPC_BAT && npm->ns->inFlight)
    {
