...
void BotUpdateInventory(bot_state_t *bs)
{
    ...
    for( i = 0; i < 9; i++ )
    {
        bs->inventory[4+i] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << i)) != 0;
    }

    //ammo
    for( i = 0; i < 9; i++ )
    {
        bs->inventory[18+i] = bs->cur_ps.ammo[i];
    }

    //powerups
    bs->inventory[INVENTORY_HEALTH] = bs->cur_ps.stats[STAT_HEALTH];
    bs->inventory[INVENTORY_SUBTANK] = bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_SUBTANK;
...
void BotBattleUseItems(bot_state_t *bs)
{
    ...
    {
        if (bs->inventory[INVENTORY_SUBTANK] > 0)
        {
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl)
{
    ...
    float attack_skill, jumper, dist, strafechange_time;
    float attack_dist = 0.0, attack_range = 0.0;
...
void BotAimAtEnemy(bot_state_t *bs)
{
    ...
    for( i = 0; i < 9; i++ )
    {
        if( wi.number == i )
        {
            aim_accuracy = trap_Characteristic_BFloat(bs->character, 8+i, 0, 1);	//8 is the starting point for weapon 0
            aim_skill = trap_Characteristic_BFloat(bs->character, 50+i, 0, 1);		//17 is the starting point for weapon 0
        }
    }
    ...
    else
    {
        ...
        if (aim_skill > 0.5)
        {
            ...
            /* Peter: Lets save this so we can use it for xbusters weapons
